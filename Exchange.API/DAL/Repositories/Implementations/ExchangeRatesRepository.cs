...
namespace Exchange.API.DAL.Repositories.Implementations
{
    public class ExchangeRatesRepository : IExchangeRatesRepository
    {
        ...
        public ExchangeRatesRepository(ApiDataContext context)
        {
            _context = context;
        }
        public async Task<decimal> GetRate(string forISO)
        {
            try
            {
                ...
                {
                    var rate = await _context.ExchangeRates.AsNoTracking().FirstOrDefaultAsync(r => r.Currency.Isocode == forISO);
